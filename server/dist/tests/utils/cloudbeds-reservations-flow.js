...
exports.goToReservationsSearchAndAuthorizeCreditCard = goToReservationsSearchAndAuthorizeCreditCard;
exports.openSideDrawerIfNeeded = openSideDrawerIfNeeded;
exports.goToReservationsAndSearch = goToReservationsAndSearch;
const test_1 = require("@playwright/test");
const logger_service_1 = require("../../src/common/logger/logger.service");
const cloudbeds_1 = require("./cloudbeds");
const logger = new logger_service_1.LoggerService('CloudbedsReservationsFlow');
function getHumanDelayConfig() {
    ...
    const postReservationsWaitMs = Number.parseInt(process.env.CLOUDBEDS_POST_RESERVATIONS_WAIT_MS ?? '0', 10);
    return {
...
async function waitBeforeTypingOnReservations(page, requestId) {
    const { reservationsBeforeTypeMs } = getHumanDelayConfig();
    if (!reservationsBeforeTypeMs || reservationsBeforeTypeMs <= 0)
        return;
    logger.logInfo('Waiting on reservations page before typing reservation id', 'CloudbedsReservationsFlow', 'waitBeforeTypingOnReservations', requestId, {
        reservationsBeforeTypeMs,
        currentUrl: page.url(),
    });
    await page.waitForTimeout(reservationsBeforeTypeMs);
}
async function humanDelay(page, requestId, reason) {
...
async function observePause(page, requestId, reason) {
    const { observeMs } = getHumanDelayConfig();
    if (!observeMs || observeMs <= 0)
        return;
...
async function clickReservationNameFromResults(page, requestId, reservationId) {
...
async function goToReservationsSearchAndAuthorizeCreditCard(page, propertyId, reservationId, amount = '10') {
    const requestId = logger.generateRequestId();
    const normalizedPropertyId = (0, cloudbeds_1.normalizePropertyId)(propertyId);
    logger.logInfo('Starting dashboard-to-reservations authorize-credit-card flow', 'CloudbedsReservationsFlow', 'goToReservationsSearchAndAuthorizeCreditCard', requestId, {
        propertyId,
    ...
    await (0, test_1.expect)(page).toHaveURL((0, cloudbeds_1.buildDashboardUrl)(normalizedPropertyId), {
        timeout: 120000,
    });
    ...
    await (0, test_1.expect)(page).toHaveURL((0, cloudbeds_1.buildReservationsUrl)(normalizedPropertyId), {
        timeout: 120000,
    });
    await observePause(page, requestId, 'after-reservations-load');
    const searchBox = getReservationsSearchInput(page);
    await (0, test_1.expect)(searchBox).toBeVisible({ timeout: 60000 });
    ...
    logger.logInfo('Completed dashboard-to-reservations authorize-credit-card flow', 'CloudbedsReservationsFlow', 'goToReservationsSearchAndAuthorizeCreditCard', requestId, {
        finalUrl: page.url(),
    });
}
async function openCreditCardsTab(page, requestId) {
...
async function clickAuthorizeButtonOnSelectedCard(page, requestId) {
...
async function authorizeCreditCardInModal(page, requestId, amount) {
    ...
    const modalTitle = (await boldTitle.count().catch(() => 0)) > 0
        ? boldTitle.first()
        : modalContent.getByRole('heading', { name: /authorize credit card/i }).first();
    await (0, test_1.expect)(modalTitle).toBeVisible({ timeout: 60000 });
    logger.logInfo('Authorize Credit Card modal is visible and title validated', 'CloudbedsReservationsFlow', 'authorizeCreditCardInModal', requestId);
    const amountInput = modalContent
        ...
        .first();
    await (0, test_1.expect)(amountInput).toBeVisible({ timeout: 60000 });
    ...
    const authorizeButton = modalContent
        ...
        .first();
    await (0, test_1.expect)(authorizeButton).toBeVisible({ timeout: 60000 });
    ...
    await (0, test_1.expect)(captureButton).toBeVisible({ timeout: 60000 });
    await (0, test_1.expect)(voidButton).toBeVisible({ timeout: 60000 });
    ...
    logger.logInfo('Clicked Authorize in modal (authorization submitted)', 'CloudbedsReservationsFlow', 'authorizeCreditCardInModal', requestId, {
        amount,
    });
}
async function clickCaptureButtonAndReadModal(page, requestId) {
    const captureButton = page.locator('[data-hook="capture-card"]').first();
    ...
    logger.logInfo('Capture modal input value read', 'CloudbedsReservationsFlow', 'clickCaptureButtonAndReadModal', requestId, {
    ...
    const cancelButton = captureModal
        ...
        .first();
    await (0, test_1.expect)(cancelButton).toBeVisible({ timeout: 60000 });
...
async function waitAfterReservationsFlow(page, requestId) {
    const { postReservationsWaitMs } = getHumanDelayConfig();
    if (!postReservationsWaitMs || postReservationsWaitMs <= 0)
        return;
...
async function openSideDrawerIfNeeded(page, requestId) {
...
function getReservationsMenuItem(page) {
    return page
...
function getReservationsSearchInput(page) {
    return page
...
async function goToReservationsAndSearch(page, propertyId, reservationId) {
    const requestId = logger.generateRequestId();
    const normalizedPropertyId = (0, cloudbeds_1.normalizePropertyId)(propertyId);
...
//# sourceMappingURL=cloudbeds-reservations-flow.js.map