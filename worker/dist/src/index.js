...
async function runCloudbedsPlaywrightAutomation(params) {
    ...
    try {
        ...
        const authFile = node_path_1.default.resolve(__dirname, '../.auth/user.json');
        ...
        const { goToReservationsAndSearch } = await Promise.resolve().then(() => __importStar(require('../tests/utils/cloudbeds-reservations-flow')));
...
async function markTaskFailed(params) {
    ...
    await prisma.chargeTask.update({
        ...
        data: {
            status: client_1.ChargeTaskStatus.FAILED,
...
async function processChargeTask(task) {
    ...
    if (payment.status === client_1.PaymentStatus.CAPTURED) {
        ...
        await prisma.chargeTask.update({
            where: { id: task.id },
            data: { status: client_1.ChargeTaskStatus.COMPLETED, completedAt: new Date() },
        });
        return;
    }
    ...
    if (result.success) {
        ...
        await prisma.chargeTask.update({
            where: { id: task.id },
            data: { status: client_1.ChargeTaskStatus.COMPLETED, completedAt: new Date(), errorMessage: null },
        });
        ...
        for (const task of tasks) {
            try {
                ...
                const claimed = await prisma.chargeTask.updateMany({
                    where: { id: task.id, status: client_1.ChargeTaskStatus.PENDING }, // Only if still PENDING
                    data: { status: client_1.ChargeTaskStatus.IN_PROGRESS, startedAt: new Date() },
                });
...
async function getTaskCounts() {
    const [pending, inProgress, completed, failed] = await Promise.all([
        prisma.chargeTask.count({ where: { status: client_1.ChargeTaskStatus.PENDING } }),
        prisma.chargeTask.count({ where: { status: client_1.ChargeTaskStatus.IN_PROGRESS } }),
        prisma.chargeTask.count({ where: { status: client_1.ChargeTaskStatus.COMPLETED } }),
        prisma.chargeTask.count({ where: { status: client_1.ChargeTaskStatus.FAILED } }),
    ]);
...
async function main() {
    ...
    const cronJob = node_cron_1.default.schedule(cronSchedule, () => {
